...
describe("My Token", () => {
    ...
    describe("approve & transferFrom", () => {
        it("should transfer from signer0 to signer1", async () => {
            ...
            await expect(myTokenC.connect(signer1).transferFrom(signer0.address, signer1.address, hre.ethers.parseUnits("10", decimals))).to.emit(myTokenC, "Transfer").withArgs(signer0.address, signer1.address, hre.ethers.parseUnits("10", decimals));

            expect(await myTokenC.balanceOf(signer1.address)).equal(hre.ethers.parseUnits("10", decimals));
        });
    });
});
